...
def main(args):
    ...
    model = Classifier(batch.size(-1)*batch.size(-1), len(rotations)).to(args.device)
    ...
    try:
        for epoch in range(start_epoch, args.epochs): 
            nll, re_loss, kl_divergence, d_loss = run(args, train_loader, model, optimizer, epoch, train=True)
            print('-' * 90)
            ...
            nll, re_loss, kl_divergence, d_loss = run(args, test_loader, model, optimizer, 1, train=False)
    ...
    nll, re_loss, kl_divergence, d_loss = run(args, test_loader, model, optimizer, epoch, train=False)    
    print('='* 90)
